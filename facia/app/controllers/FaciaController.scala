...
trait FaciaController
  ...
  /**
    * getPressedCollection returns a PressedCollection.
    *
    * Note, the way it does this is a bit circuitous. Firstly, it finds a front
    * that contains the collection (via the ConfigAgent, which is basically a
    * cache of configuration for Guardian Fronts). It then looks up that front
    * in Frontend's S3 and extracts the full collection from it (with curated
    * and backfill content etc.). It would be easier if collections were stored
    * somewhere independently of Fronts.
    */
